...
Meteor.methods({
	...
	var statsRec = Stats.findOne({ "userId" : Meteor.userId() });
	var total;
	if (statsRec !== undefined && statsRec.totalAvgTime !== undefined) total = statsRec.totalAvgTime;
	if (rec) {
	    var times = rec.times;
	    var oldAvg = rec.avg;
	    ...
	    if (total) {
		Stats.update({ "userId" : Meteor.userId() },
			     { $set : { "totalAvgTime" : total - oldAvg + avg }});
	    }
	} else {
	    ...
	    if (total) {
		Stats.update({ "userId" : Meteor.userId() },
			     { $set : { "totalAvgTime" : total - MSQ.INITIAL_TIME + time }});
	    }
	}
    }, insertTotal: function (total) {
	if (Stats.fetch({ "userId" : Meteor.userId() }).count() > 0) {
	    Stats.update({ "userId" : Meteor.userId() },
			 { $set : { "totalAvgTime" : total }});
	} else {
	    Stats.insert({ "userId" : Meteor.userId(), "totalAvgTime" : total });
	}
    }
});
